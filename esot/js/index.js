...
var _new_hexGrid = __webpack_require__(28);
...
var popup = function popup() {
	...
	return {
		main: function main() {
			var self = this;
			$(s.tab).on('mouseover', function (event) {
				event.stopPropagation();
				var id = $(this).data('id');
				// console.log('------------',id);
			...
			$('.popup-content').on('mouseover', function (event) {
				event.stopPropagation();
				// console.log('-------contetn');
				clearTimeout(timer);
			});
		...
		events: function events() {
			$(window).on('closePopUp', function () {
				$(s.content).hide();
				// console.log('close')
			});
		},
		regScroll: function regScroll() {
			$('.header__nav-link--reg').on('click', function () {
				// $(window).scrollTo('.sample')
				$([document.documentElement, document.body]).animate({
					scrollTop: $(".sample").offset().top - 20
				}, 275);
			});
		},
		init: function init() {
